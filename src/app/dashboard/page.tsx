  ...
  if (!hasItems) {
    return (
      <div className="max-w-owner mx-auto flex flex-col items-center justify-center" style={{ minHeight: '70vh', padding: '40px 0' }}>
        ...
        <h1 style={{ font: "800 28px/1.2 'Plus Jakarta Sans'", letterSpacing: '-.025em', textAlign: 'center', margin: '0 0 12px' }}>
          Let's protect your<br />first item
        </h1>
        <p style={{ font: "400 15px/1.6 'Plus Jakarta Sans'", color: 'var(--ink2)', textAlign: 'center', margin: '0 0 32px', maxWidth: 320 }}>
        ...
        <div style={{ width: '100%', maxWidth: 340, display: 'flex', flexDirection: 'column', gap: 12, marginBottom: 32 }}>
          {[
            ...
            <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 14 }}>
              <div style={{ width: 28, height: 28, borderRadius: '50%', background: 'var(--accent-soft2)', color: 'var(--accent-ink)', display: 'flex', alignItems: 'center', justifyContent: 'center', font: "700 13px 'Plus Jakarta Sans'", flexShrink: 0, marginTop: 1 }}>
                {i + 1}
              </div>
              <p style={{ font: "400 14px/1.5 'Plus Jakarta Sans'", color: 'var(--ink2)', margin: 0 }}>{step}</p>
            </div>
          ))}
        </div>
        ...
        <Link
          href="/activate"
          style={{ display: 'inline-flex', alignItems: 'center', gap: 9, padding: '14px 28px', borderRadius: 14, background: 'var(--accent)', color: 'var(--on-accent)', font: "700 15px 'Plus Jakarta Sans'", textDecoration: 'none', boxShadow: 'var(--shadow-accent)', marginBottom: 14 }}
        >
        ...
        <p style={{ font: "400 13px 'Plus Jakarta Sans'", color: 'var(--ink3)', margin: 0, textAlign: 'center' }}>
  ...
  return (
    <div className="max-w-owner mx-auto">
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
        <h1 style={{ font: "800 26px/1.2 'Plus Jakarta Sans'", letterSpacing: '-.025em', margin: 0 }}>
          Hi, {profile?.first_name || 'there'}
        </h1>
        <div style={{ width: 38, height: 38, borderRadius: '50%', background: 'var(--accent-soft2)', display: 'flex', alignItems: 'center', justifyContent: 'center', font: "700 15px 'Plus Jakarta Sans'", color: 'var(--accent-ink)' }}>
      ...
      {allProtected && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 14px', borderRadius: 12, background: '#e4f0e9', border: '1px solid rgba(58,138,100,.2)', marginBottom: 16 }}>
          ...
          <p style={{ font: "500 13px 'Plus Jakarta Sans'", color: '#2e6a4d', margin: 0 }}>
      ...
      {alertCount > 0 && (
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 14px', borderRadius: 12, background: '#f6ecd8', border: '1px solid rgba(192,138,46,.3)', marginBottom: 16 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <p style={{ font: "600 13px 'Plus Jakarta Sans'", color: '#8a5a16', margin: 0 }}>
              {alertCount} item{alertCount !== 1 ? 's' : ''} {alertCount !== 1 ? 'need' : 'needs'} your attention
            </p>
          </div>
          <Link href="/dashboard/cases" style={{ font: "600 12px 'Plus Jakarta Sans'", color: '#8a5a16', textDecoration: 'none' }}>
            View →
          </Link>
        </div>
      )}
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
        <h2 style={{ font: "700 16px 'Plus Jakarta Sans'", margin: 0 }}>Your items</h2>
        <Link
          href="/activate"
          style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '8px 14px', borderRadius: 10, background: 'var(--accent)', color: 'var(--on-accent)', font: "600 13px 'Plus Jakarta Sans'", textDecoration: 'none' }}
        >
          ...
          return (
            ...
            >
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <p style={{ font: "600 15px 'Plus Jakarta Sans'", margin: '0 0 3px', color: 'var(--ink)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                  {item.name}
                </p>
                <p style={{ font: "400 12px 'JetBrains Mono'", color: 'var(--ink3)', margin: 0 }}>
                  {item.tag?.serial ?? 'No tag linked'}
                  {isActive && (
                    <span style={{ fontFamily: "'Plus Jakarta Sans'", fontWeight: 500, color: '#c08a2e', marginLeft: 8 }}>
              ...
              <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '6px 10px', borderRadius: 999, background: chip.bg, flexShrink: 0 }}>
                ...
                <span style={{ font: "600 11.5px 'Plus Jakarta Sans'", color: chip.color, whiteSpace: 'nowrap' }}>{chip.label}</span>
              </div>
            </Link>
          )
        })}
