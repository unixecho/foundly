  ...
  return (
    <div style={{ display: 'flex', gap: 8, alignItems: 'flex-end' }}>
      ...
      <button
        ...
        style={{ height: 44, padding: '0 16px', borderRadius: 12, border: 'none', background: canSend ? 'var(--accent)' : 'var(--line2)', color: canSend ? 'var(--on-accent)' : 'var(--ink3)', font: "600 13px 'Plus Jakarta Sans'", cursor: canSend ? 'pointer' : 'default', flexShrink: 0, transition: 'all .15s', opacity: loading ? 0.7 : 1 }}
      >
        {loading ? '…' : 'Send'}
      </button>
    </div>
