  ...
  return (
    <section className="card p-5 mb-4" style={{ border: `1px solid ${!verified && currentEmail ? 'rgba(192,138,46,.3)' : 'var(--line)'}` }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 4 }}>
        <h2 style={{ font: "700 16px 'Plus Jakarta Sans'", margin: 0 }}>Emergency email</h2>
        <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, padding: '4px 10px', borderRadius: 999, font: "600 11px 'Plus Jakarta Sans'", color: statusChip.color, background: statusChip.bg }}>
          {statusChip.label}
        </span>
      </div>
        ...
        <>
          {currentEmail ? (
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 14px', borderRadius: 12, background: 'var(--surface2)', border: '1px solid var(--line)', marginBottom: 12 }}>
              <span style={{ font: "500 14px 'Plus Jakarta Sans'", color: 'var(--ink)' }}>{currentEmail}</span>
              {!verified && (
                <button
                  onClick={() => { setDraft(currentEmail); setMode('add') }}
                  style={{ border: 'none', background: 'transparent', font: "600 12px 'Plus Jakarta Sans'", color: 'var(--amber, #c08a2e)', cursor: 'pointer', padding: '4px 8px' }}
                >
                  Resend link
                </button>
              )}
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <button
              ...
              style={{ flex: 1, height: 44, border: '1px solid var(--line2)', borderRadius: 12, background: 'var(--surface)', font: "600 13px 'Plus Jakarta Sans'", color: 'var(--ink2)', cursor: 'pointer' }}
            >
            ...
            {currentEmail && (
              <button
                ...
                disabled={loading}
                style={{ height: 44, padding: '0 16px', border: '1px solid var(--error-soft2)', borderRadius: 12, background: 'var(--error-soft)', font: "600 13px 'Plus Jakarta Sans'", color: 'var(--error)', cursor: 'pointer', opacity: loading ? 0.6 : 1 }}
              >
                Remove
              </button>
            )}
      ...
      {mode === 'add' && (
        <div className="animate-reveal">
          <p style={{ font: "400 13px/1.5 'Plus Jakarta Sans'", color: 'var(--ink2)', margin: '0 0 10px' }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <button
              ...
              style={{ flex: 1, height: 46, border: 'none', borderRadius: 12, background: isValid && !loading ? 'var(--accent)' : 'var(--line2)', color: isValid && !loading ? 'var(--on-accent)' : 'var(--ink3)', font: "600 14px 'Plus Jakarta Sans'", cursor: isValid && !loading ? 'pointer' : 'default', transition: 'all .15s' }}
            >
      ...
      {mode === 'sent' && (
        <div className="animate-reveal" style={{ textAlign: 'center', padding: '8px 0' }}>
          ...
          <p style={{ font: "700 15px 'Plus Jakarta Sans'", margin: '0 0 6px' }}>Confirmation sent</p>
          <p style={{ font: "400 13px/1.5 'Plus Jakarta Sans'", color: 'var(--ink2)', margin: '0 0 16px' }}>
            Check <b>{draft}</b> and click the link to confirm. It expires in 24 hours.
          </p>
          <button
            onClick={() => setMode('view')}
            style={{ border: '1px solid var(--line2)', borderRadius: 10, background: 'var(--surface)', font: "600 13px 'Plus Jakarta Sans'", color: 'var(--ink2)', cursor: 'pointer', padding: '8px 18px' }}
          >
            Done
          </button>
        </div>
